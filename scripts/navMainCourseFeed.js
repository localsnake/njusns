...
function mainCourseFeedInitPage() {
	/* 这边设置初始界面的内容 */
...
function mainFreshmilkGetNews(prevClick, nextClick) {
	$.get(Root + 'get_page_nums.php?type=coursefeed', //获取页数的类型
		function(data) {
			sumPage = data;
			if (prevClick) {
				$.getJSON(
					Root + 'view_coursefeed.php?cur_page=' + curPage + '&lang='+ globalLanguage,
