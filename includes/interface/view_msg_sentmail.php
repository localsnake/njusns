<?php
/**
 * An Interface of user view msg sentmail
 * 
 ...
 */
  ...
  $pagesize = SentMailPageSize;		// the pagesize of every refresh
  ...
  for($i=0;$i<count($group_msgs);$i++) {
  	...
  	//if($msg_info->send_status) {	// not deleted
	if($group_id){
		...
		$result[$j]['receiver_name'] =  $receiver_info->user_name;
  		...
  		$result[$j]['content_preview'] = mb_substr(strip_tags($msg_info->content),0,20,'utf-8');
